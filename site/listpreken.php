...
foreach ($l_AFiles as $val1){
   ...
   if($val1 <> '.' && $val1 <> '..'){
		...
		$l_sfile			= $l_xmlResult->file;
         echo $l_sdatum.'<br>';
        $dispDate		= substr($l_sdatum, 6,  4) . substr($l_sdatum, 4,  2) . substr($l_sdatum, 0,  4);
        echo $dispDate.'<br>';
...
array_multisort($datum2, SORT_ASC, $tijd2, SORT_ASC  ,$aactieLijst) ;
 ...
 foreach ($aactieLijst as $key => $value) {
    ...
    // format tekst voor scherm
        ...
        <tr>
        	<td class = "' .$l_scontentcel . '" >'. $dispDate . '   ' . $l_stijd . ' </td>
        </tr>
        <tr>' . $l_sUrlMp3File . '
        </tr>
        <tr>
